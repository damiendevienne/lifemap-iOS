...
function CreatePopUps() {
	//DONE: make this function clever so that only the markers that appear or disappear are added or removed (respectively).
//we only remove markers that have their popup closed.

	//get the list of taxids already present, and set an empty list of markers to keep.
	var alreadythere=[]
	var markerstokeep=[]
	markers.eachLayer(function (markerthere) {
		alreadythere.push(markerthere.options.taxid)
	});
	z = map.getZoom() + 4;
	bb = map.getBounds();
	...
	$.ajax({
		...
		success : function(data) {
			...
			$.each(ok, function( index, value ) {
				//TODO: for each detect if it is (or not) already in the group.
				...
				var rank = str[2];
				var taxid = str[3];
				if (!(alreadythere.includes(taxid))) {
					var marker = L.marker(latlong,{icon: mark, taxid:taxid});
					...
					console.log("dejala")
					console.log(taxid)
					markerstokeep.push(taxid)
				}
			});
			var taxidstoremovenow = $(alreadythere).not(markerstokeep).get();
			markers.eachLayer(function (markerthere) {
				if (taxidstoremovenow.includes(markerthere.options.taxid)) {
					markers.removeLayer(markerthere);
				}
			});
			markers.addTo(map);
		},
	...
	// var marker = L.marker([-4.226497,0], {icon: mark, zIndexOffset:-1000, save:"nnnjnnjnj"});
	// marker.on("click", function() {
	// 				markofun("1", "LUCA", "last universal common ancestor", "root");
	// 			})
...
// function CreatePopUpsSAVE() {
// 	console.log("la")
// 	//TODO: make this function clever so that only the markers that appear or disappear are added or removed (respectively).
// //we only remove markers that have their popup closed.
// 	console.log(markers)
// 	map.removeLayer(markers); //TODO: REMOVE this
// 	console.log("li")
// 	console.log(markers)
// 	markers = new L.FeatureGroup(); //TODO: REMOVE this
// 	console.log("lu")
// 	console.log(markers)
// 	z = map.getZoom() + 4;
// 	bb = map.getBounds();
...
// 				var rank = str[2];
// 				var taxid = str[3];
// 				//TODO: if this taxid is not already in the group 'markers', then do what follows.
